...
void diplayGraph(vector<int>[],int);
...
int main()
{
    ...
    vector<int> arr[n];
    int c1,c2;
    cout<<"Enter connection as in ' a,b ' and remember node starts from 0:-"<<endl;//or 'from A:-'
    for (int i=0;i<e;i++)
    {
        scanf("%d,%d",&c1,&c2);
        addEdge(arr,c1,c2);
...
{
    arr[u].push_back(v);    //arr[u-'A'].push_back(v-'A') if you decide to enter letters, also change below line
    arr[v].push_back(u);    //comment this line for directed graph from u to v
}

void diplayGraph(vector<int> arr[],int n)
{
   ...
   //uncomment line above and comment line above the above line to use this
    ...
    for(int f=0;f<n;f++)//this is just printing all edges from each node no., therefore it can be further optimized
    {
        ...
        {
            line(coords[f].first,coords[f].second,coords[(*i)].first,coords[(*i)].second);
            //outtextxy((5*coords[*i].first + coords[f].first)/6,(5*coords[*i].second + coords[f].second)/6,"*");
